...
//eventually zipped
//ask confirmation to leave the page
...
class SQLiteXplore {
	...
	async runQuery(button) {
		...
		let {objs, cols, error} = this.performQuery(
			query, params);
		Promise.all([objs, cols])
		.then(([objs, cols]) => {
			this.showTable(queryTab, objs, cols, error, Date.now() - started);
		});
	}
	...
	showTable(queryTab, objs, colnames, error, duration=0) {
		let logRows=false;
		...
		tbody.textContent = '';
		tfoot.textContent = '';
		if (objs.length || error) {
			//let colnames = Object.keys(objs[0]);
			...
			let csvDisplay = (objs.length && colnames.length) ? 'inline-block' : 'none';
	...
	async featuresLayer(layerLabel, layerId, sldStyle, query, params={}) {
		let {objs, cols, error} = this.performQuery(query, params);
		if (error) {
			alert(error);
			return;
		}
		objs.then(async objs => {
		let extent;
		if (objs.length) {
			if (!('feature' in objs[0])) {
			...
			if (geometry) {
				let extentObj = this.performQuery(`
					...
					`, params);
				if (extentObj.error) {
					error += ' ' + extentObj.error;
				}
				else {
					extent = await extentObj.first;
				}
			}
		}
		...
		this.showLayer(
			layerId, layerLabel, objs, extent, {sldStyle});
		});
	}
	...
	async buildConditionalSchema() {
		let {first} = this.performQuery(`
			SELECT json(json_group_object(name, type))
			FROM sqlite_schema
			WHERE type IN ('table', 'view')
		`);
		let entities = await first;
		let available = Object.entries(conditionalSchema)
			...
			.map(entry => {
				let [entity, query] = entry;
				let {rows} = this.performQuery(query);
				return rows;
			});
